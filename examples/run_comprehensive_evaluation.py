#!/usr/bin/env python3
"""
Comprehensive Evaluation Script for QLLM Models.

This script demonstrates how to use the comprehensive evaluation capabilities
including advanced metrics and visualizations for thorough model assessment.
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Run comprehensive QLLM evaluation")
    
    parser.add_argument(
        "--config", "-c",
        type=str,
        default="examples/complete_evaluation_config.json",
...
def main():
    ...
    # Check for old config filename and provide backward compatibility
    if args.config == "examples/enhanced_evaluation_config.json" and not os.path.exists(args.config):
        new_config_path = "examples/complete_evaluation_config.json"
        if os.path.exists(new_config_path):
            print(f"Notice: '{args.config}' has been renamed to '{new_config_path}'")
            print(f"Using '{new_config_path}' instead.")
            args.config = new_config_path
    
    # Ensure the specified config file exists
    ...
    print(f"\nRunning comprehensive evaluation with configuration from {config_path}")
    results = run_comprehensive_evaluation(config_path, args.output_dir)
